...
PARAMETER_SECTION
    ...
    vector prj_MspB_y(1,mxYr);                                //projected male spawning biomass by year (Tier 4 proxMSY)
...
//------------ START of SHORTCUT OFL Calculations
  // First, define function for calculating Bmsy proxy, which is the Average Biomass from 1975 - yr


FUNCTION dvariable calcProxyBmsy_Shortcut(dvector MMB_survey, dvector DirCatch, dvar3_array& M_msz, double dtM, int yr, ostream& cout)
    int debug = 0;
    if (debug) {
...
//-------------OFL Calculations--------------
FUNCTION void calcOFL_Shortcut(int yr, int debug, ostream& cout)
...
//     if (debug) {
//     cout<<"#------------------------"<<endl;
//     cout<<"Shortcut method results:"<<endl;
//     cout<<"Survey-based Bmsy = "<<ptrOFLResults->BmsyProxy<<endl;
//     cout<<"Survey-based Fofl = "<<ptrOFLResults->Fofl<<endl;
//     cout<<"Survey-based OFL = "<<ptrOFLResults->OFL<<endl;
//     cout<<"#------------------------"<<endl;
//     cout<<"finished calcOFL_Shortcut(yr,debug,cout)"<<endl<<endl<<endl;
// }

//------------OFL Calculations for Operating Model-------------
//---------------EDITED BY MS 3/20/19---------------------------


FUNCTION void calcOFL_OpMod(int debug, ostream& cout)
    if (debug) {
        cout<<endl<<endl<<"#------------------------"<<endl;
...
// TAC FUNCTIONS FOR ESTIMATION MODEL

//-------------------------------------

// ---- Helper Functions for TAC Setting ------

// Get mature male biomass (MMB) in thousand tonnes
// shortcut_switch: 0=model estimates, 1=survey estimates
FUNCTION double getMMB(int shortcut_switch)
    double MMB = 0.0;
...
//  Get long-term average mature male biomass in thousand tonnes
//  Note: model-based historical estimates used UNLESS in shortcut mode

FUNCTION double getAveMMB(int shortcut_switch)
...
// Get mature female biomass at time of spawning for the year
//  Note: Always uses model-based historical estimates regardless of shortcut mode

FUNCTION double getMFB(int shortcut_switch, FleetData* ptrSurvey)
...
//  Get long-term average mature female biomass in thousand tonnes
//  Note: Always uses model-based historical estimates regardless of shortcut mode

FUNCTION double getAveMFB(int shortcut_switch, FleetData* ptrSurvey)
    int debug = 0;
    double aveMFB;
    int baseYr = 1975;
...
// Get ELMA
// Get exploitable legal male ABUNDANCE by size (20-32)
// Applies shell selectivity: 100% new shell, sOS% old shell (40% default)
// shortcut_switch: 0=model estimates, 1=survey estimates
// Returns: dvector of abundance (millions) for size bins 20-32
FUNCTION dvector getELMA(int shortcut_switch, FleetData* ptrSurvey)
    int debug = 0;
...
// Get ELMB_State (State of Alaska definition)
// Get exploitable legal male BIOMASS with shell selectivity (thousand tonnes)
// Applies 100% selectivity to new shell, 40% to old shell
// shortcut_switch: 0=model estimates, 1=survey estimates
// Returns: ELMB_State in thousand tonnes
FUNCTION double getELMB_State(int shortcut_switch, FleetData* ptrSurvey)
    int debug = 0;
...
// Get ELMB
// Get exploitable legal male BIOMASS (thousand tonnes)
// Calls getELMA() and converts to biomass using weight-at-size
// shortcut_switch: 0=model estimates, 1=survey estimates
// Returns: ELMB in thousand tonnes
FUNCTION double getELMB(int shortcut_switch, FleetData* ptrSurvey)
...
FUNCTION int calcTAC(int hcr, double OFL)  //int calcOption
...
// Harvest Control Rule 1--Female Ramp
    if (hcr==1){ 
        PRINT2B1("= Calc HCR1 =")   
        // Get biomass metrics
...
// Harvest Control Rule 2--Male Ramp
    if (hcr==2){  
        PRINT2B1("= Calc HCR2 =")   
    // Get biomass metrics
        double MMB = getMMB(ptrMOs->Shortcut);
...
// Harvest Control Rule 22--Male Ramp RAW SURVEY ESTIMATES
    if (hcr==22){         
...
// Harvest Control Rule 23--Male Ramp MODEL SURVEY ESTIMATES
    if (hcr==23){      
...
//-------------------------------------
FUNCTION double repTAC(int hcr, double OFL)  //int calcOption
...
//-------------------------------------
// Function to returned uncapped TAC to assess if ABC or OFL would be exceeded by state HCR
FUNCTION double repTAC_uncapped(int hcr, double OFL)  //int calcOption
...
// Harvest Control Rule 22--Male Ramp SURVEY ESTIMATES 
    if (hcr==22){      
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++
//
// TAC Functions for Operating Model
//
//+++++++++++++++++++++++++++++++++++++++++++++++++

FUNCTION int calcTAC_OpMod(int hcr, double OFL)  //int calcOption
...
// Harvest Control Rule 22--Male Ramp <- SURVEY BIOMASS ESTIMATES 
    if (hcr==22){  
...
// Harvest Control Rule 23--Male Ramp <- MODEL SURVEY BIOMASS ESIMATES 
    if (hcr==23){  
...
FUNCTION double repTAC_OpMod(int hcr, double OFL)  //int calcOption
...
// Harvest Control Rule 23--Male Ramp <- MODEL SURVEY BIOMASS ESITMATES 
    if (hcr==23){  
